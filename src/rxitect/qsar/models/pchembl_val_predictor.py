...
class PChEMBLValueRegressor(pl.LightningModule):
    def __init__(self, lr: float):
        ...
        self.encoder = LSTMEncoder(
            input_dim=128, encoder_dim=128, num_layers=2, dropout=0.8
        )
...
class LSTMEncoder(pl.LightningModule):
    ...
    def forward(self, inp, previous_hidden=None, pack=True):
        ...
        if pack:
            input_lengths_sorted, perm_idx = torch.sort(
                input_length, dim=0, descending=True
            )
            input_lengths_sorted = (
                input_lengths_sorted.detach().to(device="cpu").tolist()
            )
            input_tensor = torch.index_select(input_tensor, 0, perm_idx)
            rnn_input = pack_padded_sequence(
                input=input_tensor, lengths=input_lengths_sorted, batch_first=True
            )
        else:
    ...
    def init_hidden(self, batch_size):
        return torch.zeros(
            self.n_layers * self.n_directions, batch_size, self.encoder_dim
        )
