...
def main():
    ...
    randomized_smile_orderings  = [utils.randomize_smiles(mol) for _ in tqdm(range(num_random_samples), desc="Randomizing SMILES orderings")]

    # Convert all the molecules to SELFIES
    selfies_ls = [sf.encoder(x) for x in tqdm(randomized_smile_orderings, desc="Convert all molecules to SELFIES")]
    ...
    for num_mutations in num_mutation_ls: 
        # Mutate the SELFIES: 
        selfies_mut = utils.get_mutated_selfies(selfies_ls.copy(), num_mutations=num_mutations)

        # Convert back to SMILES: 
        smiles_back = [sf.decoder(x) for x in tqdm(selfies_mut, desc="Converting SELFIES back to SMILES")]
    ...
    start_time = time.time()
    canon_smi_ls_scores = utils.get_fp_scores(canon_smi_ls, target_smi=smi, fp_type=fp_type)
    print('Fingerprint calculation time: ', time.time()-start_time)
    print('Total time: ', time.time()-total_time)

    # Molecules with fingerprint similarity > 0.8
    indices_thresh_8 = [i for i,x in tqdm(enumerate(canon_smi_ls_scores), desc="Filter mols with similarity > 0.8") if x > 0.8]
