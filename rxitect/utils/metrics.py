...
def pareto_frontier(obj_vals: NDArray, maximize: bool = True):
    """
    ...
    if maximize:
        pareto_mask = pareto.is_non_dominated(torch.from_numpy(obj_vals))
    else:
        pareto_mask = pareto.is_non_dominated(-torch.from_numpy(obj_vals))
    return obj_vals[pareto_mask]


def get_hypervolume(flat_rewards: torch.Tensor, zero_ref: bool = True) -> float:
...
def r2_indicator_set(reference_points, solutions, utopian_point):
    ...
    reference points (*reference_points) and a utopian_point (*utopian_point).
        ...
        :param utopian_point: utopian point that represents the best possible solution
...
def _sharpe_ratio_qp_max(p, Q, rf):
...
def sharpe_ratio_max(p, Q, rf):
    ...
    """
    y = _sharpe_ratio_qp_max(p, Q, rf)
    x = y / y.sum()
    x = np.where(x > 1e-9, x, 0)
    sr = sharpe_ratio(p, Q, x, rf)
    return sr, x


# Assumes that l <= A << u
# Assumes A, l, u are numpy arrays
def _expected_return(A, low, up):
    """
...
def _covariance(A, low, up, p=None):
    ...
    p = _expected_return(A, low, up) if p is None else p
    Pmax = np.maximum(A[:, np.newaxis, :], A[np.newaxis, ...])
    P = _expected_return(Pmax, low, up)
...
def HSRindicator(A, low, up, managedup=False):
    ...
    p = _expected_return(A[ix], low, up)
    Q = _covariance(A[ix], low, up, p)

    hsri, x[validix[ix]] = sharpe_ratio_max(p, Q, 0)
...
def get_topk(rewards, k):
    ...
    Returns
    ----------
    average top-K rewards across all preferences
    """
