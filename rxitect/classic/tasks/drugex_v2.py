...
from rxitect.classic.models.generator_rnn import GeneratorRNN
from rxitect.classic.utils import tensor_utils
from rxitect.classic.utils.environment import Environment
from rxitect.classic.utils.vocabulary import SelfiesVocabulary


class Rxitect:
    def __init__(self, generator: GeneratorRNN,
    ...
    def policy_gradient(self,
                        crossover_net: Optional[GeneratorRNN] = None,
                        mutation_net: Optional[GeneratorRNN] = None) -> None:
        ...
        sequences = sequences[torch.tensor(uidx, device=self.device, dtype=torch.long)]

        # Scoring step
        scores = self.env.calc_reward(smiles=smiles)
        dataset = TensorDataset(sequences, torch.tensor(scores, device=self.device))
    ...
    def fit(self,
        ...
        log_filepath = here() / f"logs/rxitect_run_{time_tag}.log"
        pkg_filepath = here() / f"models/{output_filename}_{time_tag}.pkg"
        best_score = 0
        last_save = 0
...
def _init_crossover_net() -> GeneratorRNN:
    voc = SelfiesVocabulary(vocabulary_file_path=here() / "data/selfies_voc.txt")
    crossover_net = GeneratorRNN(voc=voc)
    crossover_net.load_state_dict(torch.load(here() / "models/fine_tuned_selfies_rnn.ckpt")["state_dict"])
    return crossover_net.eval()


def _init_mutation_net() -> GeneratorRNN:
    voc = SelfiesVocabulary(vocabulary_file_path=here() / "data/processed/selfies_voc.txt")
    mutation_net = GeneratorRNN(voc=voc)
    mutation_net.load_state_dict(torch.load(here() / "models/pretrained_selfies_rnn.ckpt")["state_dict"])
    return mutation_net.eval()


def evolve(net: GeneratorRNN,
           batch_size: int,
           epsilon: float = 1e-3,
           device: str = "cpu",
           crover: Optional[GeneratorRNN] = None,
           mutate: Optional[GeneratorRNN] = None, ) -> torch.Tensor:
    """
    """
    # Start tokens
    x = torch.tensor(
    ...
    for step in range(net.voc.max_len):
        ...
        #  If all the samples generation are done, stop the sampling process
        if (is_end == 1).all():
            break
    return sequences
...
def base_3_obj_run():
    voc = SelfiesVocabulary(here() / "data/processed/selfies_voc.txt")
    gen = GeneratorRNN(voc=voc)
    gen.load_state_dict(torch.load(here() / "models/fine_tuned_selfies_rnn.ckpt")["state_dict"])
