...
from rxitect.classic.utils import mol_utils, sorting, tensor_utils
from rxitect.scorers.a2ascore import Predictor
from rxitect.scorers.rascore import RAScoreNet, mol2fcfp, load_rascore_model
...
class Environment:
    ...
    def get_preds(self, mols: List[Mol],
                  use_mods: bool = True) -> pd.DataFrame:
        """
        Calculate the predicted scores of all objectives for all samples
        Args:
        ...
        Returns:
            preds (DataFrame): The scores of all objectives for all samples which also includes validity
        ...
        for objective in self.objectives:
            ...
            elif isinstance(objective.predictor, RAScoreNet):
                fcfps = [mol2fcfp(mol) for mol in mols]  # TODO: Check if this works properly
                score = objective.predictor(fcfps)
            else:
    ...
    def get_default_env(cls, scoring_scheme: ScoringScheme = ScoringScheme.PR) -> "Environment":
        A1_pred = Predictor(path=here() / "models/RF_REG_CHEMBL226.pkg")
        A2A_pred = Predictor(path=here() / "models/RF_REG_CHEMBL251.pkg")
        ERG_pred = Predictor(path=here() / "models/RF_REG_CHEMBL240.pkg")
    ...
    def get_ra_boosted_env(cls, scoring_scheme: ScoringScheme = ScoringScheme.PR, xgb: bool = False) -> "Environment":
        ...
        A2A_pred = Predictor(path=here() / "models/RF_REG_CHEMBL251.pkg")
        ERG_pred = Predictor(path=here() / "models/RF_REG_CHEMBL240.pkg")
        RA_pred = load_rascore_model(ckpt_filepath=here() / "models/rascore_26102022.ckpt")
