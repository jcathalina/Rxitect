...
def dominate(ind1: np.ndarray, ind2: np.ndarray):
    ...
    Args:
        ind1 (np.ndarray): m-d vector represented the scores of a solution for all objectives.
        ind2 (np.ndarray): m-d vector represented the scores of a solution for all objectives.
    Returns:
...
def similarity_sort(swarm, fps, is_gpu=False):
    ...
    """
    if is_gpu:
        swarm = torch.tensor(swarm, device="cuda")
...
def nsgaii_sort(swarm: np.ndarray, is_gpu: bool = False):
    ...
    """
    if is_gpu:
        swarm = torch.Tensor(swarm).to("cuda")
        fronts = gpu_non_dominated_sort(swarm)
    else:
        fronts = cpu_non_dominated_sort(swarm)
    rank = []
