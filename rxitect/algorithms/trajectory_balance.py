...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def _corrupt_actions(self, actions: List[ActionIndex], cat: ActionCategorical):
    ...
    def create_training_data_from_own_samples(
        ...
        mol_not_sane = 0
        invalid_act = 0
        logprob_of_illegal: List[torch.Tensor] = []
        ...
        for t in range(self.max_len) if self.max_len is not None else count(0):
            ...
            for i, j in zip(not_done(range(n)), range(n)):
                ...
                # Check if we're done
                if graph_actions[j].act_type is ActionType.STOP or (
                    self.max_len and t == self.max_len - 1
                ):
                    done[i] = True
                    if self.sanitize_samples and not ctx.is_valid_graph(graphs[i]):
    ...
    def compute_batch_losses(
        self, model: SamplingModel, batch: Batch, num_bootstrap: int = 0
    ):
        ...
        if self.tb_loss_is_mae:
            ...
            raise NotImplementedError("Huber loss is not supported yet")  # TODO
        else:
