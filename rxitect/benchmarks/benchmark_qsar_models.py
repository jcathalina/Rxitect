...
def cross_validate_svr(
    ...
    gs = GridSearchCV(
        SVR(),
        {"C": 2.0 ** np.array([-15, 15]), "gamma": 2.0 ** np.array([-15, 15])},
        n_jobs=-1,
    )
    ...
    for train_index, test_index in tqdm(folds, desc="Looping through folds"):
        ...
        model.fit(
            ...
            y[train_index],
            sample_weight=[
                1 if px_val >= 4 else 0.1 for px_val in y[train_index]
            ],
        )
...
def kfold_cv_benchmark(
    ...
    model.fit(
        X=X_train,
        y=y_train,
        sample_weight=[1 if px_val >= 4 else 0.1 for px_val in y_train],
    )
    ...
    joblib.dump(
        cv_result,
        filename=f"{out_dir}/cv_{k}_fold_results_{target}_{date_label()}.pkl",
    )
    ...
    for target in targets:
        if cfg.qsar_model.name == QSARModel.XGB:
            ...
            model = RandomForestRegressor(
                n_estimators=1_000, max_features="sqrt"
            )
        else:
            raise Exception(
