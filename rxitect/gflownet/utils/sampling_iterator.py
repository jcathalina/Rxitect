...
class SamplingIterator(IterableDataset):
    ...
    """
    def __init__(self, dataset: Dataset, model: nn.Module, batch_size: int, ctx: IGraphContext, algo: IGraphAlgorithm,
                 task: IGraphTask, device: torch.device, offline_ratio: float = 0.5,
                 stream: bool = True, log_dir: Optional[str] = None, sample_cond_info: bool = True):
        """Parameters
        ----------
        dataset: Dataset
            A dataset instance
