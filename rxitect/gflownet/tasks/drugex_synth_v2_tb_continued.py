...
class DrugExV2FragTrainer(BaseTrainer):
    ...
    def setup_model(self):
        ...
        if self.hps["resume"]:
            ckpt_path = self.hps["ckpt_path"]
            ckpt_data = torch.load(ckpt_path)
            state_dict = ckpt_data["models_state_dict"][0]
            model.load_state_dict(state_dict)
            print("GFN Checkpoint Loaded Successfully!")
    ...
    def default_hps(self) -> Dict[str, Any]:
        return {
            ...
            'algo': 'TB',
            'log_dir': str(here() / f'logs/mogfn/drugex_synth_v2_tb_beta_96_lr_5e-4_cnt/'),
            'num_training_steps': 5_000,
            'validate_every': 250,
            'valid_sample_cond_info': False,
            'mask_invalid_rewards': False,
            'resume': True,
            'ckpt_path': str(here() / 'logs/mogfn/drugex_synth_v2_tb_beta_96_lr_5e-4/model_state.pt')
        }
